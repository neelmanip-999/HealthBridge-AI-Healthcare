...
const COLOR_MAP = {
  ...
  yellow: { border: "border-yellow-500", iconBg: "bg-yellow-100", iconColor: "text-yellow-600" },
  ...
  teal: { border: "border-teal-500", iconBg: "bg-teal-100", iconColor: "text-teal-600" },
};

const DashboardCard = ({ title, Icon, colorKey = "indigo", link, description, onClick }) => {
  const c = COLOR_MAP[colorKey] || COLOR_MAP.indigo;
  return (
    <button onClick={onClick || (() => window.location.href = link)} type="button" className={`group w-full text-left bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-md border-l-4 ${c.border} transition-transform transform hover:-translate-y-1`}>
  ...
  return (
    <article className="bg-white border border-gray-100 rounded-xl p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm">
      ...
      <div className="flex items-center gap-3 mt-4 md:mt-0">
        <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${paymentClass}`}>{apt.paymentStatus}</span>
        <button onClick={() => onJoin(apt)} className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-4 py-2 rounded-lg shadow hover:opacity-95 transition">
          <Video className="h-4 w-4" /> Join
        </button>
        <button onClick={() => onCancel(apt._id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition border border-transparent hover:border-red-100"><Trash2 className="h-4 w-4" /></button>
      </div>
    </article>
  );
};
...
const PatientDashboard = () => {
  ...
  // CRITICAL: Destructure isConnected to know when socket is ready
  ...
  useEffect(() => {
    // Only run if socket exists, is connected, and we have a patient ID
    if (socket && isConnected && patient?._id) {
        console.log("🔵 Socket Connected! Joining Personal Room:", patient._id);
        ...
        const handleSessionRequest = (data) => {
            ...
            // You can add an Audio().play() here for a ringtone
        };
  ...
  const handleAcceptCall = () => {
    if (incomingSession) {
        ...
        navigate(`/patient/chat/${incomingSession.doctorId}`, { 
            state: { doctorData: { _id: incomingSession.doctorId, name: incomingSession.doctorName } } 
        });
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 p-6 md:p-10 relative overflow-hidden">
      ...
      {incomingSession && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center relative overflow-hidden border-2 border-green-400">
                {/* Background Pulse Effect */}
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-green-400 to-blue-500 animate-pulse"></div>
                
                <div className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 animate-bounce shadow-lg">
                    <PhoneIncoming className="w-12 h-12 text-green-600" />
                </div>
                
                <h3 className="text-2xl font-bold text-gray-900 mb-2">Incoming Call</h3>
                <p className="text-gray-600 mb-8 font-medium">Dr. {incomingSession.doctorName} is starting the consultation.</p>
                
                <div className="flex gap-4">
                    <button 
                        onClick={handleDeclineCall}
                        className="flex-1 py-3 bg-red-100 text-red-600 rounded-xl font-bold hover:bg-red-200 transition flex items-center justify-center gap-2"
                    >
                        <X className="w-5 h-5" /> Decline
                    </button>
                    <button 
                        onClick={handleAcceptCall}
                        className="flex-1 py-3 bg-green-500 text-white rounded-xl font-bold hover:bg-green-600 transition shadow-lg shadow-green-200 flex items-center justify-center gap-2 animate-pulse"
                    >
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl mb-8 border-t-4 border-green-600 shadow">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold ${isConnected ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
             <Wifi className="w-3 h-3" /> {isConnected ? 'Live' : 'Connecting...'}
          </div>
        ...
        <section className="bg-white/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl shadow border border-green-100">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="ml-auto flex items-center gap-2">
              <button onClick={fetch} className="text-sm px-3 py-1 rounded bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition">Refresh</button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-lg border-2 border-dashed border-indigo-200 text-center">
              ...
              <button onClick={() => navigate("/patient/doctors")} className="bg-indigo-600 text-white px-4 py-2 rounded-lg">Find a Doctor</button>
            </div>
          ) : (
